		...
		while ($order = mysqli_fetch_array($checkOrder)) {
			...
			if ($orderItems->num_rows > 0) {
				while ($itemDetail = mysqli_fetch_array($orderItems)) {
					...
					echo "<td><span id='newPrice'>" . money_format('%(#5n', $price * $quantity) . '</span></td>';
					echo '</tr>';
				}
			}
			...
			echo "<tr> <td colspan='2'></td><td colspan='2'> Sales Tax: " . money_format('%(#5n', $stax) . "</td></tr>";
			echo "<tr> <td colspan='2'> <td colspan='2'> Order Total: " . money_format('%(#5n', $total) . "</td><input type='hidden' id='total' value= '" . $total . "'/> </td></tr>";
			echo "<tr rowspan='2'> <td colspan='2'></td><td colspan='2'> Shipping Address: " . $address . "</td></tr>";
			echo "<tr>";
			echo " <td colspan='2'></td>";
			echo " <td colspan='2'><input type='submit' value='Confirm Order'> &nbsp; <a href ='/php/browse.php' >Continue Shopping</td>";
			echo "</tr>";
			echo '</table>';
		}
	}
