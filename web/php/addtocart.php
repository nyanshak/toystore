<?php
	...
	$_POST = array_map('trim', $_POST);
		...
		if (!empty($_SESSION['LoggedIn']) && !empty($_SESSION['Email'])) {
			...
			$query = "INSERT INTO orderitem (OrderI)";
			...
			$query = "SELECT `order`.Id FROM `order` WHERE UserId='" . $user_id . "' AND Status='pending'";
			$values = $mysqli->query($query);
			if ($values->num_rows > 0) {
				...
				$query = "INSERT INTO `order` (OrderDate, Total, Status, UserId) VALUES (CURDATE(), " . ($_POST['qty'] * $_POST['price']) . ", pending, " . $user_id . ")";
				$mysqli->query($query);
